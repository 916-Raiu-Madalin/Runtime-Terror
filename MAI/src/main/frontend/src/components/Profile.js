...
import {
    Grid, Paper, Avatar, TextField, Button, Box, Container, FormControlLabel, Typography, Alert
} from "@mui/material"
...
const Profile = () => {
    ...
    const fetchData = (event) => {
        ...
        axios({
            method: 'GET', url: '/api/profile', auth: {
    ...
    const handleSubmit = (event) => {
        ...
        axios({
            method: 'post', url: '/api/profile', headers: {
    ...
    const enrol = (event) => {
        ...
        event.preventDefault();
        axios({
            method: 'post', url: '/api/enrol', headers: {
                'Authorization': 'Basic ' + Buffer.from(localStorage.getItem('user') + ':' + localStorage.getItem('password')).toString('base64'),
                'Content-Type': 'multipart/form-data'
            }, params: {
                year: enrolYear
    ...
    return (<Container component="main" maxWidth="xl">
            ...
            <Box onSubmit={enrol} component="form" noValidate>
                <TextField
                    margin="normal"
                    fullWidth
                    id="year"
                    ...
                    name="enrol"
                    value={enrolYear || ""}
                    onChange={(event) => setEnrolYear(event.target.value)}
                    autoComplete="1"
                    autoFocus
                />
                <Button
